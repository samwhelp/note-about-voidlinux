...
``` sh
wget -c https://repo-default.voidlinux.org/live/current/void-live-x86_64-20250202-xfce.iso
```

就會下載一個檔案「`void-live-x86_64-20250202-xfce.iso`」。
...
``` sh
sudo dd if=./void-live-x86_64-20250202-xfce.iso of=/dev/sdc bs=4M status=progress && sync
```

或是執行下面指令，就會寫入「USB隨身碟」。

``` sh
sudo dd if=./void-live-x86_64-20250202-xfce.iso of=/dev/sdc bs=4M status=progress conv=fsync oflag=direct
```
